...
This project demonstrates a fullstack application using a React frontend and a LangGraph-powered backend agent. The agent is designed to perform comprehensive research on a user's query by dynamically generating search terms, performing real-time web search using Tavily API, reflecting on the results to identify knowledge gaps, and iteratively refining its research until it can provide a well-supported answer with citations. This application serves as an example of building research-augmented conversational AI using LangGraph and Qwen models via ModelScope API.
...
- 🔍 Dynamic search query generation using Qwen models via ModelScope API.
- 🌐 Real-time web research via Tavily Search API.
- 🤔 Reflective reasoning to identify knowledge gaps and refine research.
- 📄 Generates comprehensive answers with citations from real web sources.
...
-   Python 3.11+
-   **API Keys**: The backend requires the following API keys:
    - **`OPENAI_API_KEY`**: For accessing Qwen models via ModelScope API
    - **`TAVILY_API_KEY`**: For real-time web search functionality
